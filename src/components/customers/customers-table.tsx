...
export function CustomersTable({ initialCustomers }: { initialCustomers: Customer[] }) {
  ...
  const handleDelete = async (customerId: string) => {
    if (!user) {
        toast({ variant: 'destructive', title: 'Error', description: 'You must be logged in to delete a customer.' });
        return;
    }
    try {
        await deleteDoc(doc(firestore, `users/${user.uid}/customers`, customerId));
