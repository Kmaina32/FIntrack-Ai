...
export default function POSPage() {
  ...
  const productsQuery = useMemoFirebase(() =>
    user ? query(collection(firestore, `users/${user.uid}/products`)) : null
  , [firestore, user]);
  ...
  useBarcodeScanner((barcode) => {
    ...
    if (product) {
        ...
        toast({ variant: 'destructive', title: "Not Found", description: `No product found for barcode ${barcode}.`});
    }
  });
  ...
  const processSale = async () => {
    ...
    try {
        ...
        const saleData = { lineItems: cart, subtotal, tax, total, date: new Date().toISOString() };

        const result = await handleProcessSale(saleData, idToken);

        if (result.success) {
            setCompletedSale({ ...saleData, id: result.saleId, userId: user.uid });
            setCart([]);
            toast({ title: 'Sale Successful', description: 'The sale has been recorded.' });
        } else {
            throw new Error(result.error || "Failed to process sale on server");
        }
    } catch (error: any) {
        ...
        <div className="grid md:grid-cols-3 gap-8 flex-1 mt-4">
          ...
          <div className="md:col-span-2 flex flex-col">
              <Card className="flex-1 flex flex-col">
                  <CardHeader>
                      <ProductSearch products={products || []} onProductSelect={handleAddToCart} />
                  </CardHeader>
                      ...
                      <ScrollArea className="h-[calc(100vh-350px)]">
                          {cart.length === 0 ? (
                              <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
                                  ...
                                  <p>Use the search bar or a barcode scanner to add products.</p>
                              </div>
                          ): (
          ...
          <div className="md:col-span-1">
            <Card className="h-full flex flex-col">
              ...
              <CardFooter>
                 <Button className="w-full" size="lg" disabled={cart.length === 0 || isProcessing} onClick={processSale}>
